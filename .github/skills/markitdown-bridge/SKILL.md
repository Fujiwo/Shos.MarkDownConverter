...
description: 'Use when implementing, reviewing, or refactoring Shos.MarkDownConverter MarkItDown integration, Python environment checks, external process execution, or supported-format decisions.'
...
## Goals
- Keep Shos.MarkDownConverter as a web application wrapper around MarkItDown.
- Isolate Python and process execution details behind testable abstractions.
- Preserve a clear boundary between web request orchestration and document conversion.

## Procedure
1. Confirm whether the task changes upload flow behavior, environment probing, or only the bridge implementation.
...
5. Map failures to stable HTTP responses, logs, and actionable error messages.
